...
try:
    ...
    print(f"{Fore.RED}❌ Failed to load config: {e}{Style.RESET_ALL}" if ENABLE_COLORS else f"Error: Failed to load config: {e}")
    config = {}
    ENABLE_COLORS = True
    DEBUG_MODE = False
    MAX_RETRIES = 3
...
class RealTimeTranscriber:
    ...
    def __init__(self, model_name: str = None, device: Optional[int] = None):
        ...
        self.chunk_duration_sec = config.get("defaults", {}).get("chunk_duration_sec", 1.0)
    ...
    def load_model(self) -> bool:
        if self.model is None:
            ...
            if not self._is_model_cached(self.model_name):
                print(f"{Fore.YELLOW}⚠️ Model not found in cache - will download{Style.RESET_ALL}")
            else:
                print(f"{Fore.GREEN}✅ Model found in cache{Style.RESET_ALL}")

            use_fp16 = self.device_type in ("cuda", "mps")
            device_name = {"cuda": "NVIDIA GPU", "mps": "Apple Silicon", "cpu": "CPU"}[self.device_type]
            print(f"{Fore.GREEN}⚡ Using {device_name} ({'fp16' if use_fp16 else 'fp32'}){Style.RESET_ALL}")
    ...
    def _transcription_worker(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                if self._is_speech(audio_buffer) or \
                   len(audio_buffer) >= 1.5 * self.sample_rate * self.chunk_duration_sec:
                    ...
                    result = self.model.transcribe(audio_buffer, fp16=(self.device_type in ("cuda", "mps")))
                    ...
                    if text:
                        print(f"\r{Fore.GREEN}{time.strftime('%H:%M:%S')}: {text}{Style.RESET_ALL}", end="", flush=True)
        ...
        if self.is_running:
            ...
            if hasattr(self, 'stream') and self.stream:
                try:
                    ...
                    print(f"{Fore.RED}⚠️  Error stopping audio stream: {e}{Style.RESET_ALL}")
            
            # Wait for worker thread to finish
            if hasattr(self, 'worker_thread') and self.worker_thread:
                self.worker_thread.join(timeout=2.0)
                if self.worker_thread.is_alive():
                    print(f"{Fore.RED}⚠️  Worker thread did not stop gracefully{Style.RESET_ALL}")
    ...
    def run(self) -> None:
        ...
        try:
            ...
            print(f"{Fore.CYAN}🎤 Starting real-time transcription (press Ctrl+C to stop)...{Style.RESET_ALL}")
...
def select_model_and_run():
    ...
    if custom_models:
        ...
        for i, name in enumerate(custom_models, len(MODELS)+1):
            print(f"  {i}. {name.replace('-', ' ').title()} {Fore.GREEN}(installed){Style.RESET_ALL}")
    ...
    while True:
        try:
            ...
            if 1 <= choice <= len(MODELS):
                ...
                break
            print(f"{Fore.RED}⚠️ Please enter a number between 0 and {len(MODELS)+len(custom_models)}.{Style.RESET_ALL}")
        except ValueError:
